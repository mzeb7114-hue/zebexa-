// auth.js - FIXED VERSION (no auto-redirect)
...
// Check existing session on page load - BUT NO REDIRECT
checkSession();
...
async function checkSession() {
  try {
    ...
    if (session) {
      // User is already logged in - just log it, don't redirect
      console.log("User is already logged in:", session.user.email);
      // Optionally show a welcome message in the login form
      message.textContent = `Welcome back, ${session.user.email}. Sign in again or continue.`;
      return true;
    }
    return false;
  } catch (error) {
